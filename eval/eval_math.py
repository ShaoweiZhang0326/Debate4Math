...
def test_hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    ...
    stop_tokens = []
    sampling_params = SamplingParams(temperature=0, top_p=1, max_tokens=15000, stop=stop_tokens)
    ...
    results = []
    res_dict = []
    # for idx, (problem, prompt, completion, prompt_answer, subject) in enumerate(zip(problem_list, hendrycks_math_ins, res_completions, hendrycks_math_answers, subject_list)):
    for idx, (problem, prompt, completion, prompt_answer) in enumerate(zip(problem_list, hendrycks_math_ins, res_completions, hendrycks_math_answers)):
        ...
        res_dict.append(ele)
    with open('your jsonl path here', 'w', encoding='utf-8') as f:
    ...
    print('length====', len(results), ', acc====', acc, 'correct_numbers===', sum(results))

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default='your model path here')  # model path
